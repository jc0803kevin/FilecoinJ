...
package io.filecoin.crypto.blake2b;
...
public class Bench implements Runnable {
    ...
    static class Usage {
        ...
        static int usage() {
            System.out.println("usage: java -cp .. icc.multisig.wallet.fil.ove.crypto.digest.Bench [options]");
    ...
    public static Call newCallBlake2b(final int size) {
        final Blake2b.Digest digest = Blake2b.Digest.newInstance(new Blake2b.Param().setDigestLength(size));
        return new Call() {
            @Override
            final public byte[] func(byte[] b) {
                digest.reset();
